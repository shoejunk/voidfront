...
Sim::Sim(uint32_t seed, uint32_t count) : rng_(seed ? seed : 1) {
...
void Sim::apply(const Command& c) {
    ...
    for (const auto id : c.units) {
        ...
        u.order = c.order;
        u.target_id = 0;
        // Keep the edge endpoint reserved while stopped between centers. A later move resumes it.
        if (c.order == Order::Stop || c.order == Order::Hold) {
            u.goal_x = u.next_x; u.goal_z = u.next_z;
            continue;
        }
...
int Sim::route(const Unit& u, int goal) const {
    const int start = cell_of(u.x, u.z);
    std::array<bool, cells> occupancy{};
    for (const auto& other : units_) if (other.hp > 0 && other.id != u.id) {
        occupancy[cell_of(other.x, other.z)] = true;
        occupancy[cell_of(other.next_x, other.next_z)] = true;
    }
    std::array<int, cells> parent;
    parent.fill(-1);
    std::array<int, cells> queue{};
    int head = 0, tail = 1, closest = start;
    queue[0] = start; parent[start] = start;
    // Static ordering resolves equal routes. Occupancy includes both endpoints of moving units.
    while (head < tail) {
        const int here = queue[head++];
        if (manhattan(here, goal) < manhattan(closest, goal)) closest = here;
        if (here == goal) { closest = here; break; }
        const std::array<int, 4> neighbors{here - kMapWidth, here - 1, here + 1, here + kMapWidth};
        for (const auto next : neighbors) {
            if (next < 0 || next >= cells || parent[next] >= 0 || manhattan(next, here) != 1 ||
                blocked(next % kMapWidth, next / kMapWidth) || occupancy[next]) continue;
            parent[next] = here; queue[tail++] = next;
        }
    }
    if (closest == start) return start;
    while (parent[closest] != start) closest = parent[closest];
    return closest;
}
...
void Sim::step() {
    ...
    for (auto& u : units_) {
        ...
        int goal = cell_of(u.goal_x, u.goal_z);
        if (target) {
            u.target_id = target->id;
            if (nearest <= attack_range2) {
                if (u.cooldown == 0) { damage[target->id - 1] += 8; u.cooldown = 10; }
                goal = cell_of(u.x, u.z);
            } else if (u.order == Order::AttackMove) goal = cell_of(target->x, target->z);
        }
        if (u.order == Order::Stop || u.order == Order::Hold) continue;
        const bool at_center = u.x == u.next_x && u.z == u.next_z;
        if (at_center && (u.order == Order::Move || u.order == Order::AttackMove)) {
            const int next = route(u, goal);
            u.next_x = center_x(next); u.next_z = center_z(next);
        }
        // Axis-aligned grid edges; fixed 32/256 world units per tick (2.5 units/s).
        const int dx = std::clamp(u.next_x - u.x, -32, 32);
        const int dz = std::clamp(u.next_z - u.z, -32, 32);
        u.x += dx; u.z += dz; u.moving = dx != 0 || dz != 0;
