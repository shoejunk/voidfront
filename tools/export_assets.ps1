...
& $blender --background --factory-startup --python (Join-Path $repo 'art/generate_walker.py')
if ($LASTEXITCODE -ne 0) { throw 'Blender asset generation failed' }
& $blender --background --factory-startup --python (Join-Path $repo 'art/validate_walker.py')
